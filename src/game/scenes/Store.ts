...
export class Store extends Scene {
    ...
    async loadUpgradesFromDatabase(successCallback: Function, failureCallback: Function) {
        try {
            const response = await axios.get('https://rutold.onrender.com/:10000/gameData/upgrades');
    ...
    async displayUpgrades(upgrades) {
        ...
        for (const [index, upgrade] of upgrades.entries()) {
            ...
            try {
                const response = await axios.get(`https://rutold.onrender.com/:10000/User/upgrades/${user.userid}/${upgrade.id}`, {
            ...
            if (!userOwnsCharacter) {
                ...
                ((currentTier) => {
                    buyButton.on('pointerdown', async () => {
                        try {
                            await axios.post(`https://rutold.onrender.com/:10000/User/${user.userid}/upgrades/${upgrade.id}`, {}, {
                                headers: { 'Authorization': `Bearer ${jwtToken}` }
                            });
                            upgradeText.destroy();
                            counter++
